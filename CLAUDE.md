...
> **깃허브에 커밋을 요청하면 변경된 모든 내용(.gitignore에 들어갈만한 내용을 제외한 모든 내용, .gitignore에 들어갈만 한데 아직 들어가있지 않다면 꼭 확인 요청할것)을 add할것** 
> **commit 명령어 실행 시 어떤 메세지를 포함할건지만 물어보고 가장 기본적인 commit 명령어를 활용할 것**
> **claude를 공동작업자로 변경할 것인지에 대한 물음을 하지 않을 것(앞으로 절대 claude에게 공동작업자 요청할 일 없음)** 
> **이후 push까지 기본적인 깃허브 커밋 명령어만 실행할 것.**
...
3. **분석**: DA3(메인: 포즈+depth+3D메시) → SAM2 → Florence-2 → Voxel → 3D 구조 우선순위 지도 (COLMAP은 fallback)
...
│   │   └── loader.py
│   ├── pipeline/
│   │   ├── reconstruct.py     # DA3 메인 (포즈+depth+3D메시) + COLMAP fallback
│   │   ├── depth.py           # DA3 Dense Depth + Point Cloud export
│   │   ├── mesh_export.py     # DA3 → glb/ply 3D 메시 export
...
DA3: Large (~4GB), use_ray_pose=True, export_format="glb"
SAM2: Large | DA3 배치: 4 | COLMAP: fallback 전용
...
순차 실행: DA3-Large(~4GB, 메인: 포즈+depth+3D메시) → SAM2(~2GB) → Florence-2(~2GB) = ~8GB
COLMAP(~1GB)은 DA3 신뢰도 낮을 때만 fallback 실행
...
| 2 | AI 파이프라인 | preprocess→DA3(포즈+depth+3D)→SAM2→Florence-2→fuse→voxel→visualize |
...
*RESCUE-EYE CLAUDE.md v6.1 — 2026 KAIST 캔위성 경연대회*
